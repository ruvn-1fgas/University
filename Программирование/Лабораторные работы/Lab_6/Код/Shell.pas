...
begin
	...
	i := length(cp) + 1;
    diff := Length(cmd) - Length(cp) - sp_k;
    sp_k := 0;
    cnt := 0; k := -1;
...
begin
    ...
    if (Length(tokens) = 0) or (Length(tokens) >=6) then exit;
    _cmd := tokens[0];
    pp := 0;
    ...
    begin
        writeln('Нераспознанная команда: ', _cmd);
        err := 0;
        exit;
    end;
    ...
    begin
        ...
        if j <> 0 then
        begin //32767
            writeln('Неверный аргумент: ', tokens[i]);
            err := i;
            exit;
        end;
    end;
    ...
    begin
        writeln('Неверное кол-во аргументов');
        err := 0;
        exit;
    end;
    err := -1;
